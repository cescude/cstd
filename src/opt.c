...
size _findValIdx(opts_config_t config, str_t name) {
    /*
      If name is something like `name=xyz`, we want to drop the
...
str_t _maybeEqValue(str_t arg) {
    str_t name = strTakeToByte(arg, '=');
    if (strEndsWith(name, strC("="))) {
        return (str_t){
            .beg = name.end,
            .end = arg.end,
        };
        ...
        if (strEquals(arg, strC("--"))) {
            ...
            if (a < num_opts) {
                *opts[a].ptr.b = 1;
                ...
                str_t eq_val = _maybeEqValue(arg);
              
                /*
                  If arg looks like `arg=value`, eq_val will be
                  used. Otherwise, nextarg is used and idx will be
                  incremented.
                */
                if (!_setVal(opts[b], eq_val, nextarg, &idx)) {
                    return 0;
                }
            } else {
